@echo off
REM Builds dist\Rough Cut-Setup.exe with Inno Setup.
REM Requires: dist\Rough Cut.exe (run Build-exe.bat first),
REM           ffmpeg.exe on PATH, and Inno Setup 6 installed.
cd /d "%~dp0"

if not exist "dist\Rough Cut.exe" (
...
copy /y "dist\Rough Cut.exe" "%STAGE%\" >nul
...
"%ISCC%" "installer\Roughcut.iss"
echo.
echo Done. Installer is dist\Rough Cut-Setup.exe