...
def _stub(Rough Cut, **attrs):
    app_cls = Rough Cut["App"]
...
def test_coordinate_roundtrip(Rough Cut):
    s = _stub(Rough Cut, src_w=1920, src_h=1080, scale=0.5,
...
def test_recompute_letterbox(Rough Cut):
    class StubCanvas:
    ...
    s = _stub(Rough Cut, src_w=1920, src_h=1080, canvas=StubCanvas())
