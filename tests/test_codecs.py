...
def test_venc_h264_software(Rough Cut):
    assert Rough Cut["_venc"](mk(Rough Cut, fmt="mp4", hw=False)) == \
        ["-c:v", "libx264", "-preset", "medium", "-crf", "22"]


def test_venc_h264_gpu(Rough Cut):
    a = Rough Cut["_venc"](mk(Rough Cut, fmt="mp4", hw=True))
    assert a[:2] == ["-c:v", "h264_nvenc"] and "-cq" in a and "22" in a


def test_venc_hevc_software_tags_hvc1(Rough Cut):
    a = Rough Cut["_venc"](mk(Rough Cut, fmt="hevc", hw=False))
    assert "libx265" in a and "-crf" in a and "22" in a
    assert a[a.index("-tag:v") + 1] == "hvc1"


def test_venc_hevc_gpu_tags_hvc1(Rough Cut):
    a = Rough Cut["_venc"](mk(Rough Cut, fmt="hevc", hw=True))
    assert "hevc_nvenc" in a and "-cq" in a and "22" in a
    assert a[a.index("-tag:v") + 1] == "hvc1"


def test_venc_av1_software(Rough Cut):
    assert Rough Cut["_venc"](mk(Rough Cut, fmt="av1", hw=False)) == \
        ["-c:v", "libsvtav1", "-preset", "6", "-crf", "22"]


def test_venc_av1_gpu_when_capable(Rough Cut):
    a = Rough Cut["_venc"](mk(Rough Cut, fmt="av1", hw=True, av1_nvenc=True))
    assert a[:2] == ["-c:v", "av1_nvenc"] and "-cq" in a and "22" in a


def test_venc_av1_gpu_falls_back_to_software(Rough Cut):
    # GPU toggle on but the GPU can't do AV1 NVENC -> software libsvtav1
    a = Rough Cut["_venc"](mk(Rough Cut, fmt="av1", hw=True, av1_nvenc=False))
    assert "libsvtav1" in a and "av1_nvenc" not in a


# ---- target-size support predicate ----------------------------------------

def test_target_size_supported_only_mp4(Rough Cut):
    f = Rough Cut["_target_size_supported"]
...
def test_build_commands_hevc(Rough Cut):
    cmds = Rough Cut["build_commands"](mk(Rough Cut, fmt="hevc", output_path="out.mp4"))
    assert len(cmds) == 1
    j = " ".join(cmds[0])
...
def test_build_commands_av1(Rough Cut):
    cmds = Rough Cut["build_commands"](mk(Rough Cut, fmt="av1", output_path="out.mp4"))
    j = " ".join(cmds[0])
    assert "libsvtav1" in j and "aac" in j
    assert cmds[0][-1] == "out.mp4"


def test_build_commands_av1_ignores_target_size(Rough Cut):
    cmds = Rough Cut["build_commands"](
        mk(Rough Cut, fmt="av1", target_size_mb=10, output_path="out.mp4"))
    j = " ".join(cmds[0])
...
def test_build_commands_hevc_adds_watermark_input(Rough Cut, tmp_path):
    wm = tmp_path / "w.png"
    wm.write_bytes(b"x")
    cmds = Rough Cut["build_commands"](
        mk(Rough Cut, fmt="hevc", watermark_path=str(wm), output_path="out.mp4"))
    assert str(wm) in " ".join(cmds[0])   # watermark added as a 2nd input


# ---- per-format Export-tab control visibility -----------------------------

def test_format_controls_matrix(Rough Cut):
    f = Rough Cut["_format_controls"]
