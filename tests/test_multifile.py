def test_clip_from_info_spans_whole_file(Rough Cut):
    info = {"w": 1920, "h": 1080, "dur": 12.5, "rotation": 90,
            "fps": 30000 / 1001, "has_audio": False}
    c = Rough Cut["clip_from_info"]("a.mp4", info)
    assert c.path == "a.mp4"
...
def test_clip_from_info_defaults_fps_when_missing(Rough Cut):
    c = Rough Cut["clip_from_info"]("a.mp4", {"w": 640, "h": 480, "dur": 3.0})
    assert c.fps == 30.0
    assert c.has_audio is True


def test_batch_out_name_suffix_and_numbering(Rough Cut, tmp_path):
    taken = set()
    p1 = Rough Cut["_batch_out_name"](str(tmp_path), "C:/x/clip.mp4", ".mp4", taken)
    assert p1.replace("\\", "/").endswith("clip_export.mp4")
    # same stem again -> auto-numbered, never overwrites
    p2 = Rough Cut["_batch_out_name"](str(tmp_path), "D:/y/clip.mov", ".mp4", taken)
    assert p2.replace("\\", "/").endswith("clip_export_2.mp4")
    p3 = Rough Cut["_batch_out_name"](str(tmp_path), "E:/z/clip.avi", ".mp4", taken)
    assert p3.replace("\\", "/").endswith("clip_export_3.mp4")

def test_batch_out_name_distinct_stems(Rough Cut, tmp_path):
    taken = set()
    a = Rough Cut["_batch_out_name"](str(tmp_path), "a.mp4", ".mp4", taken)
    b = Rough Cut["_batch_out_name"](str(tmp_path), "b.mp4", ".mp4", taken)
    assert a.endswith("a_export.mp4")
    assert b.endswith("b_export.mp4")


def test_combine_target_largest_and_fps(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a", 1280, 720, 5, fps=30, end=5),
             Clip("b", 1920, 1080, 5, fps=60, end=5)]
    assert Rough Cut["_combine_target"](clips) == (1920, 1080, 60.0)

def test_combine_target_uses_per_clip_crop(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a", 1920, 1080, 5, end=5, crop=(0, 0, 640, 480))]
    assert Rough Cut["_combine_target"](clips)[:2] == (640, 480)

def test_combine_target_applies_scale_cap(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a", 1280, 720, 5, fps=30, end=5),
             Clip("b", 1920, 1080, 5, fps=60, end=5)]
    W, H, F = Rough Cut["_combine_target"](clips, scale_cap=1280)
...
def test_concat_two_clips_join_and_canvas(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a.mp4", 1280, 720, 5, fps=30, end=5),
             Clip("b.mp4", 1920, 1080, 4, fps=30, end=4)]
    cmds = Rough Cut["build_concat_commands"](clips, _g(Rough Cut))
    assert len(cmds) == 1
    cmd = cmds[0]
...
def test_concat_crop_only_on_cropped_clip(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a.mp4", 1920, 1080, 5, end=5, crop=(0, 0, 640, 480)),
             Clip("b.mp4", 1920, 1080, 5, end=5)]
    j = " ".join(Rough Cut["build_concat_commands"](clips, _g(Rough Cut))[0])
    assert "crop=640:480:0:0" in j
    assert j.count("crop=640:480") == 1            # only the first clip

def test_concat_silent_clip_gets_anullsrc(Rough Cut):
    Clip = Rough Cut["Clip"]
...
def test_concat_mute_drops_audio(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a.mp4", 1920, 1080, 5, end=5),
             Clip("b.mp4", 1920, 1080, 5, end=5)]
    j = " ".join(Rough Cut["build_concat_commands"](clips, _g(Rough Cut, mute=True))[0])
    assert "concat=n=2:v=1:a=0" in j
    assert "-an" in j
    assert "anullsrc" not in j

def test_concat_global_eq_after_join(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a.mp4", 1920, 1080, 5, end=5),
             Clip("b.mp4", 1920, 1080, 5, end=5)]
    j = " ".join(Rough Cut["build_concat_commands"](clips, _g(Rough Cut, brightness=0.1))[0])
    assert "eq=brightness=0.100" in j
    assert j.index("concat=") < j.index("eq=brightness")   # global, after join

def test_concat_webm_codecs(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a.webm", 1280, 720, 5, end=5),
             Clip("b.webm", 1280, 720, 5, end=5)]
    j = " ".join(Rough Cut["build_concat_commands"](clips, _g(Rough Cut, fmt="webm"))[0])
    assert "libvpx-vp9" in j and "libopus" in j

def test_concat_ignores_reverse(Rough Cut):
    Clip = Rough Cut["Clip"]
    clips = [Clip("a.mp4", 1920, 1080, 5, end=5),
             Clip("b.mp4", 1920, 1080, 5, end=5)]
    j = " ".join(Rough Cut["build_concat_commands"](clips, _g(Rough Cut, reverse=True))[0])
    assert "reverse" not in j   # neither video 'reverse' nor audio 'areverse'

def test_concat_watermark_overlay(Rough Cut, tmp_path):
    Clip = Rough Cut["Clip"]
    wm = tmp_path / "logo.png"
    wm.write_bytes(b"PNG")
    clips = [Clip("a.mp4", 1920, 1080, 5, end=5),
             Clip("b.mp4", 1920, 1080, 5, end=5)]
    cmd = Rough Cut["build_concat_commands"](
        clips, _g(Rough Cut, watermark_path=str(wm), watermark_pos="tr"))[0]
    j = " ".join(cmd)
