"""
Rough Cut - a simple native Windows UI for ffmpeg.
...
# Brand palette, extracted from the Rough Cut logo (Rough Cut.svg): warm near-black
# and marigold gold.
GOLD       = "#4FA8FF"
GOLD_LIGHT = "#7EC1FF"
GOLD_DEEP  = "#2E7FD9"
...
ICON_FILE = resource_path("Rough Cut.ico")
...
def _drawtext_filter(s):
    ...
    if not txt.strip():
        return []
    tf = os.path.join(tempfile.gettempdir(), f"Rough Cut_text_{os.getpid()}.txt")
    try:
...
def _gif_passes(s):
    ...
    dur = max(0.001, s.end - s.start)
    palette = os.path.join(tempfile.gettempdir(), f"Rough Cut_pal_{os.getpid()}.png")
...
def fetch_latest_tag(url=LATEST_API, timeout=8):
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Rough Cut"})
...
class App(BaseTk):
    ...
    def _apply_theme(self):
        ...
        # the Rough Cut logo. Replaces sv-ttk, whose accent can't be recoloured.
    ...
    def _engine_download_done(self, ok, msg):
        if ok and self._enable_mpv():
            ...
            self.play_hint.config(text="Installed — restart Rough Cut to play")
        else:
    ...
    def _show_about(self):
        ...
        ttk.Label(pad, text="Rough Cut", font=("Segoe UI", 22, "bold"),
        ...
        ttk.Label(pad, text="Rough Cut is MIT-licensed; bundled ffmpeg is GPLv3.",
    ...
    def _gen_strip(self, w, token):
        ...
        out = os.path.join(tempfile.gettempdir(),
                           f"Rough Cut_strip_{os.getpid()}.png")
